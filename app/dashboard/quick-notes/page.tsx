...
import { deleteNote } from '@/app/dashboard/action' // Reuse the delete action
...
// Helper: Show domain only (e.g. "supabase.com")
...
export default async function QuickNotesPage({ searchParams }: { searchParams: { q?: string } }) {
  const supabase = await createClient()

  // 1. Fetch Notes (With optional search filtering)
  let query = supabase
    .from('notes')
    .select('*')
    .order('created_at', { ascending: false })

  // Simple text search if ?q= is present in URL (handled by SearchBar)
  // Note: For advanced vector search, we usually use the RPC function, 
  // but for a simple "Quick Notes" list, a text match is often enough.
  ...
  return (
    <main className="w-full max-w-6xl mx-auto space-y-8 pb-10 px-4 md:px-0">
      
      {/* 1. Header & Search */}
      <div className="flex flex-col md:flex-row justify-between items-end gap-4 border-b pb-6">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 tracking-tight">All Notes</h1>
          <p className="text-gray-500 mt-1">Your entire second brain, organized.</p>
        </div>
        ...
        {notes?.map((note) => {
             ...
             return (
               <div key={note.id} className="break-inside-avoid group relative bg-white rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-all flex flex-col overflow-hidden mb-4">
                  ...
                  {!note.file_type?.startsWith('image/') && hasRichData && (
                    <div className="w-full relative bg-gray-50 border-b border-gray-100">
                      <img src={note.link_meta!.image!} alt="Link Preview" className="w-full h-48 object-cover" />
                    </div>
                  )}
          ...
          {(!notes || notes.length === 0) && (
             <div className="col-span-full py-20 text-center">
                ...
                <p className="text-gray-500 mt-1">Try searching for something else or add a new note.</p>
             </div>
          )}
        </div>
